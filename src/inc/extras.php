...
function wpg_body_class($class) {
	...
	$class[] = 'hfeed site';
 	...
 	// Active sidebar - 2 column (content)
  	if (!is_front_page() && is_active_sidebar( 'wpg-sidebar-right' ) ) {
...
function wpg_post_class($class) {
	...
	elseif (is_singular( array( 'post_gallery', 'offer' )) || is_post_type_archive('offer')) {
		$class[] = '';
	} elseif (is_category() && !is_active_sidebar( 'wpg-sidebar-right' )) {
		$class[] = 'col-9 offset-3';
	} elseif (is_category() && is_active_sidebar( 'wpg-sidebar-right' )) {
		$class[] = 'col-full-margin';
	}
...
function wpg_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {

	$attr['sizes'] = '(max-width: 321px) 320px, (max-width: 769px) 768px, (max-width: 1450px) 1440px, 100vw';

	return $attr;
}
//add_filter( 'wp_get_attachment_image_attributes', 'wpg_post_thumbnail_sizes_attr', 10, 3 );
